...
`manifest.json` makes this work without a build step). The current manifest
targets Thunderbird 153.0. The small privileged Experiment is required because
standard WebExtension APIs cannot open an existing message in Thunderbird's
native editable Draft mode.
